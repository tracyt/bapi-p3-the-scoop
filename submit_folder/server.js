...
function saveDatabase() {
  ...
  yaml.writeSync('../saved_db.yaml', database, 'utf8', 
    function(err, data){
  if (err) throw err;
...
function loadDatabase() {
  database = yaml.readSync('../saved_db.yaml', 'utf8',
    function(err, data){
  if (err) throw err;
  console.log(data.split('\n').length-1);
});
...
function createComment(url, request) {
  ...
  if (requestComment && request.body && database.articles[requestComment.articleId] && requestComment.body &&
  ...
  /// REMOVE
  // loopThrough(database,1);

  return response;
}

/*
function loopThrough(obj,level){
  var space = "";
